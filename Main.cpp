...
void Intitialize(Object* stage, int w, int h, const char* data);
...
int main()
{
	...
	Intitialize(stage, gStageWidth, gStageHeight, gStageData);
...
void Update(Object* stage, char input, int w, int h)
{
	...
	{
		std::cerr << "Invalid player postition" << std::endl;
		return;
	}
	...
	{
		stage[tp] = (stage[tp] == OBJ_GOAL) ? OBJ_BLOCK_ON_GOAL : OBJ_PLAYER;
	...
	else if (stage[tp] == OBJ_BLOCK || stage[tp] == OBJ_BLOCK_ON_GOAL)
	{
		...
		{
			std::cerr << "invalid block postition" << std::endl;
			return;
		}
		...
		if (stage[tp2] == OBJ_SPACE || stage[tp2] == OBJ_GOAL)
		{
			...
			stage[tp] = (stage[tp] == OBJ_BLOCK_ON_GOAL) ? OBJ_BLOCK_ON_GOAL : OBJ_PLAYER;
			stage[p] = (stage[p] == OBJ_PLAYER_ON_GOAL) ? OBJ_GOAL : OBJ_SPACE;
		}
	}
}
